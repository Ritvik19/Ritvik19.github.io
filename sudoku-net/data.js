...
let project_contents = {
    ...
    "Method": [
        ...
        {"type": "image", "src": "sudokunet-architecture.png", "alt": "SudokuNet Architecture",},
        {"type": "text", "content": "SudokuNet utilizes a Feed Forward Neural Network architecture, comprising two linear layers with 64 units each, followed by 81 output branches. ReLU activation is applied to the hidden layers to introduce non-linearity. Softmax activation is used in the output layer to predict the digit values for each cell in the Sudoku grid. "},
        {"type": "heading", "content": "Hyperparameters"},
        {"type": "text", "content": "The model is trained using the Adam optimizer with a learning rate of 0.001. A batch size of 64K is used for training, and the model is trained in two stages. The loss function employed is the categorical cross-entropy loss, which is suitable for multi-class classification tasks. Early stopping is applied to prevent overfitting, and the model checkpoints are saved based on the validation loss."},
