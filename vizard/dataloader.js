...
function get_example_block(title, url) {
  return `<li><a target="_blank" href="${url}">${title}</a></li>`;
}

function get_examples_contents(examples) {
  let contents = "";
  for (let x of examples) {
    contents += get_example_block(x[0], x[1]) + "\n";
  }
  return contents;
}
...
container.innerHTML += get_section(
  "line1",
  "Installation",
  get_code_block("pip install vizard")
);
container.innerHTML += get_section("line2", "Usage", get_usage(usage));
container.innerHTML += get_section(
  "line3",
  "Examples",
  `<ul style="list-style-type: circle">${get_examples_contents(examples)}</ul>`
);
